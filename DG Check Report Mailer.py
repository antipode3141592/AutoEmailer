...
path_contactlist = "//concordia/users/skirkpatrick/Export//"

#path_input = "C:\\Users\\skirkpatrick\\Documents\\Work for Art\\DG Pledge Notification Reports\\Input\\"
path_input = "C:\\Users\\skirkpatrick\\Coding\\Python\\Outgoing\\"
path_output = "C:\\Users\\skirkpatrick\\Coding\\Python\\Processed\\"
#path_output = "C:\\Users\\skirkpatrick\\Documents\\Work for Art\\DG Pledge Notification Reports\\Output\\"
path_errors = "C:\\Users\\skirkpatrick\\Coding\\Python\\Errors\\"
...
file_contactlist = path_contactlist + "funds and emails.xlsx"

#load data
message_body = open(file_emailbody).read()  #load message body from file
wb = pd.ExcelFile(file_contactlist)
df1 = wb.parse('funds_and_emails')  #parse contents of excel file into Pandas dataframe
...
for f in os.listdir(path_input):
    ...
    for index, row in df1.iterrows():   #iterate over all rows, searching for matches
        #print("org " + row[0])
        if sterilizestring(row[0]) == sterilizestring(_f[0]):             #if Org Name matches File Name, add e-mail to list
            for i in [4,5,6,7]:
                #print(row[i])
                if row[i] == row[i]:
                    emaillist.append(row[i])
                    validemailacount += 1
    if validemailacount == 0:    #if no valid matches, print error and save filename in error list
        ...
        mail.To = stringemail
        mail.Subject = 'Work for Art - Designated Fund Quarterly Payment Report'
